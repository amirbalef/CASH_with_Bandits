...
class GenericMAB:
    ...
    def MaxSearch_Gaussian(self, T, c=1.0):
        """
        Implementation of the MaxSearch_Gaussian strategy
        :param T: time Horizon
        :param alpha: hyperparameter
        """
        tr = TrackerMax(
            self.nb_arms, T, store_max_rewards_arm=True, store_rewards_arm=True
        )
        t = 0
        while t < self.nb_arms:
            tr.update(t, t, self.MAB[t].sample()[0])
        ...
        while t < T:
            ...
            for i  in range(self.nb_arms):
                ...
                if tr.Na[i] > 1:
                    mu = np.mean(tr.rewards_arm[i])
                    sigma = (np.sum( np.asarray(tr.rewards_arm[i]) ** 2) - tr.Na[i] * mu**2) / (
                        tr.Na[i] - 1
                    )
                    alpha = t**(-c**2)
                    
                    mu_hat = mu + stats.t.ppf(1 - alpha / 2, tr.Na[i] - 1) * np.sqrt(
                        sigma / tr.Na[i]
                    )

                    sigma_hat = (
                        (tr.Na[i] - 1) * sigma / stats.chi2.ppf(alpha / 2, tr.Na[i] - 1)
                    )

                    policy[i] = np.sqrt(sigma_hat / 2) * self.ierfc(
                        (tr.max_rewards_arm[i] - mu_hat) / np.sqrt(2 * sigma_hat)
                    )
                else:
                    policy[i] = np.inf   
    ...
    def MaxSearch_SubGaussian(self, T, c=1.0 / np.sqrt(13.613)):
        """
        Implementation of the MaxSearch_Gaussian strategy
        :param T: time Horizon
        :param alpha: hyperparameter
        ...
        while t < T:
            policy = np.zeros(self.nb_arms)
            for i in range(self.nb_arms):
                beta = c * np.sqrt(np.log(t) / tr.Na[i] )
                gamma = -(beta**2) + 2 * np.sqrt(2) * beta
                if gamma > np.log(2):
                    policy[i] = np.inf
                else:
                    mu = np.mean(tr.rewards_arm[i])
                    sigma = (np.mean( np.asarray(tr.rewards_arm[i]) ** 2) - mu**2)/(2 *(np.log(2) - gamma))
                    policy[i] = np.sqrt(2 * np.pi * sigma) * special.erfc(
                        (tr.max_rewards_arm[i] - mu) / np.sqrt(2 * sigma)
                    )
            policy = np.nan_to_num(policy)
            k = rd_argmax(policy)
            reward = self.MAB[k].sample()[0]
    ...
    def MaxUCB(self, T, alpha=0.5):
        """
        Implementation of the MaxUCB strategy
        :param T: time Horizon
        :param alpha: hyperparameter
        """
        tr = TrackerMax(self.nb_arms, T, store_rewards_arm=True, store_max_rewards_arm=True)
        t = 0
        while t < self.nb_arms:
            tr.update(t, t, self.MAB[t].sample()[0])
            t += 1
        while t < T:
            policy = np.zeros(self.nb_arms)
            for i  in range(self.nb_arms):
                pad = (alpha *np.log(t)/tr.Na[i])**2
                policy[i] = tr.max_rewards_arm[i]/np.max(tr.max_rewards_arm) + pad
            k = rd_argmax(policy)
    ...
    def OptimalPolicy(self, T, optimal_arm):
        """
        Implementation of the ExUCB strategy
        :param T: time Horizon
        :param alpha: hyperparameter
        """
        tr = TrackerMax(
            self.nb_arms, T, store_rewards_arm=True, store_max_rewards_arm=True
        )
        t = 0
        while t < T:
            reward = self.MAB[optimal_arm].sample()[0]
            tr.update(t, optimal_arm, reward)
            t += 1
        return tr
