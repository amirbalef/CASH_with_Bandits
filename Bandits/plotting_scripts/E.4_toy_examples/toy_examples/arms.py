...
class ArmPower(AbstractArm):
    def __init__(self, alpha, random_state=0):
        """
        :param alpha: float, exponent in Power distribution
        :param random_state: int, seed to make experiments reproducible
        """
        self.alpha = alpha
        super(ArmPower, self).__init__(
            mean=alpha / (alpha - 1),
            variance=alpha**2 / ( ((alpha - 1) ** 2) * (alpha - 2)),
            random_state=random_state,
        )
    ...
    def sample(self, size=1):
        ...
        :return: float, a sample from the arm
        """
        return self.local_random.power(self.alpha, size=size)

class ArmGenPower(AbstractArm):
    def __init__(self, alpha, scale, random_state=0):
        """
        :param alpha: float, exponent in Power distribution
        :param random_state: int, seed to make experiments reproducible
        """
        self.alpha = alpha
        self.scale = scale
        super(ArmGenPower, self).__init__(
            mean=alpha / ((alpha - 1) * self.scale**self.alpha),
            variance=alpha**2
            / (((alpha - 1) ** 2) * (alpha - 2) * self.scale**self.alpha),
            random_state=random_state,
        )
    ...
    def sample(self, size=1):
        ...
        :return: float, a sample from the arm
        """
        return self.local_random.power(self.alpha, size=size) / self.scale**self.alpha
