...
params = {
    "MaxSearch_Gaussian": {"c": 1.0},
    "MaxSearch_SubGaussian": {"c": 1.0 / np.sqrt(13.613)},
    "MoMax_SDA": {
        "fe": lambda x: max(5, np.sqrt(x)),
        "batch_size": lambda n: np.sqrt(n),
    },
    "ExtremeETC": {"b": 1},
    "ExtremeHunter": {"steps": 1000, "D": 1e-4, "E": 1e-4, "b": 1},
    "Uniform_Exploration": {},
    "Threshold_Ascent": {"s": 100, "delta": 0.1},
    "MaxMedian": {"explo_func": lambda t: 1 / t},
    "QoMax_SDA": {
        "fe": lambda r: max(5, np.log(r) ** (3 / 2)),
        "batch_size": lambda n: n ** (2 / 3),
        "q": 0.9,
    },
    "QoMax_ETC": {"size": (100, 10), "q": 0.9},
    "MaxUCB": {"alpha": 0.5},
    "OptimalPolicy": {"optimal_arm": 0},
}
...
# xp3 = {'arms': ['Exp'] * 10, 'coef': [2.1, 2.4, 1.9, 1.3, 1.1, 2.9, 1.5, 2.2, 2.6, 1.4]}  # xp 3 from Bhatt et al.
# xp4 = {'arms': ['G'] * 20, 'coef': [[1, sig] for sig in [1.64, 2.29, 1.79, 2.67, 1.70, 1.36, 1.90, 2.19,  0.80, 0.12, 1.65, 1.19, 1.88, 0.89, 3.35, 1.5, 2.22, 3.03, 1.08, 0.48]]}  # xp 3 from Bhatt et al.
xp9 = {
    "arms": ["NegExp"] * 10,
...
algs = [
    ...
    "ExtremeHunter",
    "Threshold_Ascent",
    "MaxSearch_SubGaussian",
    "MaxSearch_Gaussian",
    "OptimalPolicy",
]  # Selection of algorithms to test
...
T_list = [
   ...
   1000,
   2000,
]  # Sub-selection of times for testing
...
if __name__ == '__main__':
    for i, xp in enumerate(xp_list):
        for T in T_list:
            ...
            params["MaxUCB"]["alpha"] = 0.5
            params["OptimalPolicy"]["optimal_arm"] = optimal_arms_xp[i]
