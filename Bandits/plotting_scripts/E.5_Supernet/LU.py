...
fig, axs = plt.subplots(3, number_of_arms)
print(axs.shape)
...
for i in range(number_of_arms):
    ...
    for d in range(len(instances)):
        all_stds = np.asarray(
            [np.std(dataset[d, :, i, :].flatten()) for i in range(number_of_arms)]
        )
        ...
        for eps in epsilon:
            if eps <= 1 - np.quantile(data, q=0.99) or eps >= 1 - np.quantile(
                data, q=0.01
            ):
                res.append(np.nan)
            else:
    ...
    if dataset_name == "SubSupernet":
        ...
        axs[0, i].set_ylim(top=40, bottom=0.3)
